...
class BinaryLogSearch(object):
    def __init__(self, log_path, timestamp_format, timestamp_length, search_timestamp, line_delimiter="\n"):
    ...
    def show_found(self, lines_before=0, lines_after=0):
        ...
        while cur_pos >= 0 and found_lines_before <= lines_before:
            ...
            found = os.read(self.log_fd, len(self.line_delimiter))
            if found == self.line_delimiter:
                found_lines_before += 1
            cur_pos -= 1
        
        output_start = cur_pos + 1 + len(self.line_delimiter)
        ...
        output_length = cur_pos - 1 + len(self.line_delimiter) - output_start
...
def main():
    with BinaryLogSearch("log.txt", "%d/%b/%Y:%H:%M:%S", 20, "21/Jan/1970:18:56:39") as log_search:
